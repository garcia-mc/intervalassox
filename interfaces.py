...
class BaseLassoNet(BaseEstimator, metaclass=ABCMeta):
    def __init__(
        ...
        *,
        ...
        if optim is None:
            optim = (
                partial(torch.optim.Adam, lr=5e-5),
                partial(torch.optim.SGD, lr=5e-5, momentum=0.9),
            )
    ...
    def _train(
        ...
        *,
        ...
        def validation_obj():
            ...
            with torch.no_grad():
               ...
               # if ((epoch % 20 == 0) and denseflag):
                ...
                self.criterion_val = self.mnonpar_train.loss_val
                    
        		   

                return (
                    self.criterion_val(model(X_val),y_val).item()
        ...
        print('VALIDATION LOSS {}'.format(best_val_obj))
        ...
        for epoch in range(epochs):
            ...
            model.train()
            if ((epoch % 20 == 0) and denseflag):
                oldcumulhaz=copy.copy(self.mnonpar_train.JJ[6,:])
                ...
                Jinford=self.mnonpar_train.JJ
                newcumulhaz=self.mnonpar_train.JJ[6,:]
                distance=newdistance
                newdistance=np.mean((oldcumulhaz-newcumulhaz)**2)
                print(newdistance)
                ...
                predi0=torch.zeros(len(Jinford[6,:]),X_train.shape[1])
                q0=np.multiply(np.asmatrix(Jinford[6,:]).transpose(),np.exp(model.forward(predi0).detach().numpy()))
            ...
            if val_obj < self.tol*best_val_obj: #self.tol * best_val_obj:
            ...
            if patience is not None and epochs_since_best_val_obj == patience:
                break
            print('VALIDATION LOSS {}'.format(val_obj))
            print(epochs_since_best_val_obj)
            ...
            with torch.no_grad():
                model.eval()
                print(torch.sum(torch.where(torch.abs(param)<1e-4,1.,0.)))
            VALOBJECTIVES.append(val_obj)
           # plt.plot(VALOBJECTIVES,'-o')
            #plt.show()
    ...
    def path(self, X, y, *, X_val=None, y_val=None) -> List[HistoryItem]:
        ...
        for current_lambda in lambda_seq:
            np.savetxt('results/{}.txt'.format(current_lambda),[current_lambda])
